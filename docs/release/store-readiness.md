...
- The release gap reporter automatically reads the latest checked-in non-secret provider inventory at `docs/release/evidence/provider-secret-inventory-*.json`. To refresh that evidence without exposing secret values, run `gh secret list --repo MohamedRed/Sports-Green-Moove --json name,updatedAt`, replace the inventory artifact, then run `npm run report:release-gaps`. A temporary inventory can still be passed with `npm run report:release-gaps -- --secret-inventory /tmp/sgm-github-secrets.json`.
...
- Production provider configuration for Firebase, Radar, Google Maps Platform, and Stripe Connect must pass `npm run validate:provider-readiness` with production secrets in the environment.
- Release Readiness runs a `Production provider secret validation` job against GitHub Actions secrets for the current PR branch; passing this proves secret shape only, not provider dashboard restrictions or real-device behavior.
- Google Sign-In and Meta Facebook Login are postponed outside the Android email/password release scope. Validate them later with `npm run validate:provider-readiness -- --include-postponed-social-auth` after adding `SGM_GOOGLE_REVERSED_CLIENT_ID`, `SGM_FACEBOOK_APP_ID`, and `SGM_FACEBOOK_CLIENT_TOKEN`.
...
- Production configuration evidence for Firebase, Radar, Google Maps Platform, and Stripe Connect. Do not include secrets in the manifest.
- The manifest must keep postponed social-auth readiness explicit until Google Sign-In and Meta Facebook Login are reintroduced.
...
- The hosted admin build now exposes a public `/privacy` route without Firebase admin auth, and `npm run test:privacy-policy-page` verifies the route content and responsive layout. The production URL `https://sports-green-moove-prod.web.app/privacy` was published through Firebase Hosting REST API release `sites/sports-green-moove-prod/releases/1781710861740000` and live-verified with `HTTP/2 200` on 2026-06-17. The Firebase CLI user credential still requires `firebase login --reauth` before future CLI deploys.
- Public privacy/data-safety answers stay aligned with `docs/release/privacy-data-safety.md`.
- Safety-audit exports must be generated from a real ride run with `npm run export:safety-audit-evidence -- --ride-session-id <rideSessionId> --booking-id <bookingId> --report-id <stripeReportId>`, then validated with `npm run validate:safety-audit-export -- docs/release/evidence/safety-audit-export.json` before the manifest marks audit items accepted.
